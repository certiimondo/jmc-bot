...
async def say(ctx, color: str, *, message):
    """Bot posts an embed in the current channel with dynamic color and GIF thumbnail"""
    ...
    # Add GIF as thumbnail (top-right corner)
...
async def post(ctx, channel: discord.TextChannel, color: str, *, message):
    """Bot posts an embed in a specific channel with dynamic color and GIF thumbnail"""
    embed_color = get_color(color)
    embed = discord.Embed(
        description=message,
        color=embed_color
