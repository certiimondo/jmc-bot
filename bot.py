...
async def send_embed(ctx, color, message, gif_pos=None, channel=None):
    """Helper function to send embed with optional GIF positions and multi-line messages"""
...
# !say command
@bot.command()
async def say(ctx, color: str, gif_pos: str = None, *, message: str):
    """Send an embed in current channel with optional GIF positions and multi-line support"""
    await send_embed(ctx, color, message, gif_pos)

# !post command
@bot.command()
async def post(ctx, channel: discord.TextChannel, color: str, gif_pos: str = None, *, message: str):
    """Send an embed in specific channel with optional GIF positions and multi-line support"""
